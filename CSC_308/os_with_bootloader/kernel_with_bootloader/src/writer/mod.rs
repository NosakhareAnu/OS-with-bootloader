...
/// Padding from the border to prevent text from being too close to the edges.
const BORDER_PADDING: usize = 1;

/// Retrieves the raster of the given character or a fallback if unavailable.
...
impl FrameBufferWriter {
    ...
    /// Writes a single character to the framebuffer, handling control characters.
    ...
    /// Renders a character in the framebuffer and updates cursor position.
    ...
    fn write_pixel(&mut self, x: usize, y: usize, intensity: u8) {
        ...
        let color = match self.info.pixel_format {
            ...
            other => {
                // Ensure we have a valid pixel format before panicking to avoid double panic.
